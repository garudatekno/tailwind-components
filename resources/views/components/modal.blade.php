...
<div x-data="{ 'showModal': false }" @keydown.escape="showModal = false" x-cloak>
    ...
    <div x-show="showModal" @click.away="showModal = false" class="fixed inset-0  z-40 hidden" :class="{ 'block': showModal, 'hidden':  !showModal }">
      ...
      <div class="relative  flex items-center mx-auto h-screen pt-5 pb-4 {{$size}}">
        ...
        <div class="relative shadow rounded bg-white sm:rounded-md sm:overflow-hidden w-full">
           {{-- <i @click="showModal = !showModal" class="far fa-times-circle text-lg text-pink-500 absolute top-6 right-5"></i> --}}
            {!!$cardHeader!!}
            ...
            <div class="px-4 py-3 flex items-center space-x-2 justify-end bg-gray-100 text-right sm:px-6 " >
                {{$footer}} 
                <x-button type="button" @click="showModal = !showModal" hasColor="pink">Close</x-button>
            </div>
        </div>
